...
| `/api/schema`, `/api/query?sql=`, `/api/caveats?answer=`, `/api/glossary` | the same tools as GET routes returning JSON, plus the term definitions the UI shows on hover, in English and a dozen-odd other languages | anything that only has a web fetch; `/openapi.json` describes them |

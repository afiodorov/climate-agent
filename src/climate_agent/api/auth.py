...
a conversation, and that used to be open to anyone with the URL. Now it needs
an admin: a GitHub login named in `ADMIN_GITHUB_USERS`. Nothing else changes
for a visitor — asking, reading the rail, the MCP surface are all as before.
...
to GitHub, `/auth/callback` swaps the code for the login name, and the name is
kept in a cookie signed with HMAC so it cannot be forged. No token is stored:
the only thing this service ever wanted to know is who you are.

Configuration, all environment variables:

  GITHUB_CLIENT_ID, GITHUB_CLIENT_SECRET   a GitHub OAuth app. The callback here
                                           is /oauth2/callback so the same app
                                           as the staging edge's oauth2-proxy
                                           serves: GitHub only requires the
                                           registered callback's domain and
                                           path prefix to match, subdomains
                                           may differ.
...
USER_COOKIE = "climate_user"
STATE_COOKIE = "climate_oauth_state"
# Shared with oauth2-proxy's registered callback path; see the module docstring.
...
def _key() -> bytes:
    ...
    return hashlib.sha256(f"climate-user-cookie:{secret}".encode()).digest()
